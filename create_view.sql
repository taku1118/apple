-- マイページ用
    CREATE VIEW Personal_Inform AS SELECT student_number, user_name, COALESCE(nickname,'----'), COALESCE(my_comment,'----'), CASE gender WHEN 0 THEN "男性" WHEN 1 THEN "女性" END AS 'gender', COALESCE(birthday,'----/--/--'), COALESCE(graduate_date,'----/--/--'), school_name, course_name, CASE job_hunt WHEN 0 THEN "就活中" WHEN 1 THEN "就活終了" END AS 'job_hunt', CASE job_offer WHEN 0 THEN "内定なし" WHEN 1 THEN "内定なし" END AS 'job_offer' FROM Users JOIN Courses USING (course_id) JOIN Schools ON Courses.school_id = Schools.school_id;
